...
pub fn run() {
    // Check for headless mode and custom data directory
    let args: Vec<String> = std::env::args().collect();
    let is_headless = args.iter().any(|arg| arg == "--headless");
    
    // Parse --data-dir argument for multi-instance support
    // Usage: AIOLauncher-Server-Trans.exe --data-dir "C:\path\to\data"
    for i in 0..args.len() {
        if args[i] == "--data-dir" && i + 1 < args.len() {
            std::env::set_var("AIOLAUNCHER_DATA_DIR", &args[i + 1]);
            break;
        } else if args[i].starts_with("--data-dir=") {
            let value = args[i].strip_prefix("--data-dir=").unwrap_or("");
            if !value.is_empty() {
                std::env::set_var("AIOLAUNCHER_DATA_DIR", value);
            }
            break;
        }
    }
